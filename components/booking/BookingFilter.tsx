// components/booking/BookingFilter.tsx
'use client';
...
interface BookingFilterProps {
  ...
  onStatusChange: (status: string | BookingStatus) => void;
  onDateChange: (date: string) => void;
}

const STATUS_TABS = [
  { label: 'Tất cả', value: 'All' },
  { label: 'Chờ duyệt', value: BookingStatus.Pending },
  { label: 'Đã xác nhận', value: BookingStatus.Confirmed },
  { label: 'Đã Check-in', value: BookingStatus.CheckedIn },
  { label: 'Đã Check-out', value: BookingStatus.CheckedOut }
];

export const BookingFilter = ({ onSearch, onStatusChange, onDateChange }: BookingFilterProps) => {
  const [activeTab, setActiveTab] = useState<string | BookingStatus>('All');
  const [selectedDate, setSelectedDate] = useState(new Date().toISOString().split('T')[0]);
  const dateInputRef = useRef<HTMLInputElement>(null);

  const handleTabSelect = (value: string | BookingStatus) => {
    setActiveTab(value);
    ...
    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 w-full">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
        <div className="relative w-full sm:w-64">
          ...
          <input 
            type="text" 
            placeholder="Tìm mã đặt phòng, khách..." 
        ...
        <div className="relative w-full sm:w-auto">
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-between bg-[#E5E5EA] dark:bg-[#1C1C1E] hover:bg-[#D1D1D6] dark:hover:bg-[#2C2C2E] rounded-full px-4 py-2 transition-all duration-200 active:scale-95 text-xs font-semibold text-[#1C1C1E] dark:text-white cursor-pointer select-none"
          >
      ...
      {/* Segmented Control Lọc trạng thái */}
