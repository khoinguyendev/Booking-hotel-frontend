      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-sm divide-y divide-[#E5E5EA]/60 dark:divide-[#2C2C2E]/50 text-[#1C1C1E] dark:text-[#F2F2F7] transition-colors">
            {records.map((row) => (
              <tr key={row.id} onClick={() => handleRowClick(row.id)} className="hover:bg-[#F2F2F7]/50 dark:hover:bg-[#2C2C2E]/40 transition-colors duration-200">
                <td className="p-4 pl-6 font-mono text-xs text-[#8E8E93]">{row.id}</td>
                <td className="p-4 font-semibold text-current">{row.name}</td>
                <td className="p-4 text-xs text-[#8E8E93]">{row.department}</td>
                <td className="p-4 text-center font-mono text-xs">{row.checkIn}</td>
                <td className="p-4 text-center font-mono text-xs">{row.checkOut}</td>
                <td className="p-4 pr-6 text-right">
                  <span className="inline-flex items-center space-x-1.5 text-xs font-medium">
                    <span className={`w-2 h-2 rounded-full ${
                      row.status === 'Ontime' ? 'bg-[#34C759]' : 
                      row.status === 'Late' ? 'bg-[#FF9500]' :   
                      'bg-[#FF3B30]'                             
                    }`} />
                    <span>
                      {row.status === 'Ontime' ? 'Đúng giờ' : row.status === 'Late' ? 'Đi muộn' : 'Vắng mặt'}
                    </span>
                  </span>
                </td>
              </tr>
