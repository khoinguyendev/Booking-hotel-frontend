...
import { RequestNotification } from "@/types/RequestNotification";
...
export default function AdminLayout({
  ...
  //Dùng chung toàn bộ page
  useEffect(() => {
    let connection: HubConnection | null = null;

    const handleNewStaffRequest = (data: RequestNotification) => {
    ...
    const startSignalR = async () => {
      ...
      try {
        ...
        connection.on("NewStaffRequest", handleNewStaffRequest);
    ...
    return () => {
      connection?.off("NewStaffRequest", handleNewStaffRequest);
    };
  }, []);
  React.useEffect(() => {
    setMounted(true);
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative">
              ...
              >
                ...
                <ChevronDown
                  className={`w-3 h-3 text-slate-400 transition-transform duration-200 ${isUserMenuOpen ? "rotate-180" : ""}`}
                />
              </button>
