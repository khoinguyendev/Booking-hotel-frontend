...
export default function MainLayout({
  ...
  // ĐÃ THÊM: Quản lý vòng đời mount để tránh Hydration Mismatch
  ...
  const sidebarItems = [
    ...
    {
      name: 'Khám phá Chi nhánh',
      href: '/trang-chu/khach-san',
      icon: Hotel,
    },
    {
      name: 'Lịch sử đặt phòng',
      href: '/trang-chu/lich-su-dat-phong',
      icon: Calendar,
    },
    {
      name: 'Hồ sơ cá nhân',
      href: '/trang-chu/hồ-sơ-cá-nhân',
      icon: User,
    },
  ];

  // Xử lý thông tin hiển thị an toàn sau khi Client đã sẵn sàng
  const userFullName = mounted && user ? user.fullName || 'Tài khoản' : 'Tài khoản';
  const userEmail = mounted && user ? user.email || 'Chưa cập nhật' : 'Chưa cập nhật';
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center justify-between px-4 py-3 rounded-2xl text-sm font-semibold transition-all duration-200 group ${isActive
        ...
        <div className="p-4 border-t border-[#E5E5EA] dark:border-[#2C2C2E] space-y-3">
          <div className="flex items-center space-x-3 p-2 bg-[#F2F2F7]/50 dark:bg-[#2C2C2E]/40 rounded-2xl">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold truncate">{userFullName}</p>
              <p className="text-[10px] text-[#8E8E93] truncate">{userEmail}</p>
            </div>
          </div>
            ...
            {sidebarItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setIsSidebarOpen(false)}
                  className={`flex items-center space-x-3 px-4 py-3 rounded-2xl text-sm font-semibold transition-all ${isActive ? 'bg-[#007AFF] text-white' : 'text-[#8E8E93] hover:bg-[#F2F2F7] dark:hover:bg-[#2C2C2E]'
                    }`}
                >
